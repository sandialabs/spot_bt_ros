"""Depth camera related subscribers."""
...
from spot_bt_ros_node.cameras import ROS_DEPTH_CAMERA_TOPICS
from spot_bt_ros_node.cameras import ROS_DEPTH_CAMERA_TOPICS_WITH_ARM


def create_depth_camera_children(
    has_arm: False,
    ...
    Args:
      has_arm (bool): Whether the current Spot model used with this code base has an
        arm. This will add one additional depth camera topic.

    Returns:
      out (list[py_trees_ros.subscribers.ToBlackboard]): A list of actions that capture
        depth images for all of Spot's cameras.
    """
    if has_arm:
        return [
            subscribers.ToBlackboard(
                ...
                blackboard_variables={f"camera_depth_{camera}": None},
            )
